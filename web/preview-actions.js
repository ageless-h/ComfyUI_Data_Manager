import{_ as f,g as $,a as T,F as m,b as u,e as d,c as x,u as L,L as _,d as E,l as M,f as S,h as z}from"./extension.js";import{v as N,s as R,o as U,i as X,r as G,n as J,j as Y,m as W,k as K,q as Q,p as Z,w as ee,t as te}from"./extension.js";import"../../scripts/app.js";function H(e){if(typeof e!="string")return String(e??"");let t=e;const i=/function\s*\(\s*\)\s*\{[\s\S]*?\}/g;t=t.replace(i,"");const n=/function\s*\(\s*\)\s*\{\s*(return\s+)?[\w.]*\.?(?:_?target|value)\s*(?:\.call\([^)]*\))?[\s;]*\}/g;t=t.replace(n,"");const a=/\{[^}]*_target[^}]*\}/g;return t=t.replace(a,""),t}async function O(e){const{FileManagerState:t}=await f(async()=>{const{FileManagerState:l}=await import("./extension.js").then(p=>p.x);return{FileManagerState:l}},[]),n=(await f(()=>import("./extension.js").then(l=>l.x),[])).FileManagerState;n.currentPreviewFile=e;const a=document.getElementById("dm-preview-content"),s=document.getElementById("dm-open-floating-preview-btn");if(!a)return;a.innerHTML=`
    <div class="dm-panel-loading" style="text-align: center; padding: 20px;">
      <i class="pi pi-spin pi-spinner"></i>
    </div>
  `;const o=$(e),c=e.split(/[/\\]/).pop()||"",y=T(o);try{let l="",p=!1;if(m.image.exts.includes(o)){const r=u(e),v=`dm-panel-image-${Date.now()}`;l=`
      ...
      `,p=!0,a._imageId=v}else if(m.audio.exts.includes(o)){const r=u(e);l=`
      ...
      `,p=!0}else if(m.video.exts.includes(o)){const r=u(e),v=`dm-panel-video-${Date.now()}`;l=P(v,r),p=!0}else if(m.videoExternal?.exts.includes(o)){const r=o.toUpperCase().replace(".","");l=`
      ...
      `,p=!0}else if(m.code.exts.includes(o)){const r=await fetch(u(e));if(r.ok){const v=await r.text(),{highlightCode:g}=await f(async()=>{const{highlightCode:w}=await import("./extension.js").then(b=>b.y);return{highlightCode:w}},[]);l=k(v,o,g),p=!0}else throw new Error("Failed to load file")}else if(m.document.exts.includes(o)){const r=u(e);l=await F(e,o,r),p=!0}else if(m.spreadsheet.exts.includes(o))l=await D(e,o),p=!0;else{const r=x(),v=m[y]?.icon||m.unknown.icon,g=m[y]?.color||m.unknown.color;l=`
        <div style="text-align: center; padding: 30px;">
          <i class="pi ${v}" style="font-size: 64px; color: ${g};"></i>
      ...
      `}a.innerHTML=l,m.video.exts.includes(o)&&C(a),m.document.exts.includes(o)&&I(a),s&&(s.style.display="block",s.onclick=async()=>{const{openFloatingPreview:r}=await f(async()=>{const{openFloatingPreview:v}=await import("./extension.js").then(g=>g.A);return{openFloatingPreview:v}},[]);r(e,c)}),L(`预览: ${c}`),j(e)}catch{const p=x();a.innerHTML=`
      <div style="text-align: center; padding: 20px; color: ${p.errorColor};">
...
... (文件过大，已截断)`:e;return`
  ...
  `}async function F(e,t,i){const n=E(e);if(t===".md")return`
      <div style="display: flex; flex-direction: column; gap: 0;">
    ...
    `;if(t===".docx")try{const s=await fetch(i);if(!s.ok)throw new Error("Failed to load file");const o=await s.arrayBuffer();await M("https://cdnjs.cloudflare.com/ajax/libs/mammoth/1.6.0/mammoth.browser.min.js");const c=window.mammoth;if(!c)throw new Error("mammoth.js not available");const l=(await c.convertToHtml({arrayBuffer:o})).value,p=H(l),r=`dm-doc-content-${Date.now()}`;return`
        <div style="display: flex; flex-direction: column; gap: 0; height: 100%;">
          <div id="${r}" class="dm-docx-content"
               style="flex: 1; overflow-y: auto; padding: 20px; box-sizing: border-box;">
            <style>
              #${r} img { max-width: 100%; height: auto; display: inline-block; margin: 10px 0; }
              #${r} p { word-wrap: break-word; overflow-wrap: break-word; margin: 0.5em 0; }
              #${r} table { max-width: 100%; overflow: auto; display: block; margin: 10px 0; }
            </style>
            ${p}
          </div>
          <div class="dm-doc-controls" style="display: flex; align-items: center; justify-content: center; gap: 8px; padding: 10px;">
      ...
      `}catch(s){return console.error("[DataManager] DOCX preview error:",s),`
      ...
      <div style="text-align: center; padding: 40px;">
        <i class="pi pi-file-word" style="font-size: 64px; color: ${x().textSecondary};"></i>
        <div style="margin-top: 15px;">${d(n)}</div>
  ...
  `}async function D(e,t){const{parseSpreadsheet:i}=await f(async()=>{const{parseSpreadsheet:n}=await import("./extension.js").then(a=>a.z);return{parseSpreadsheet:n}},[]);try{const n=await i(e,t),{createTableHTML:a}=await f(async()=>{const{createTableHTML:s}=await import("./extension.js").then(o=>o.z);return{createTableHTML:s}},[]);return a(n,{type:"panel",path:e})}catch(n){return`
      <div style="text-align: center; padding: 40px; color: ${x().errorColor};">
        <i class="pi pi-exclamation-triangle" style="font-size: 48px;"></i>
        <div style="margin-top: 15px;">表格解析失败</div>
        <div style="margin-top: 5px; font-size: 11px;">${d(n.message)}</div>
      </div>
    `}}async function j(e){const t=document.getElementById("dm-file-info");if(t)try{const i=await S(e);t.innerHTML=`
