...
describe('SSH API', () => {
  ...
  describe('sshConnect', () => {
    ...
    it('should handle authentication failure', async () => {
      ...
      await expect(sshConnect('test.example.com', 22, 'testuser', 'wrongpass')).rejects.toThrow(
        '认证失败: 用户名或密码错误'
      )
    })
    ...
    it('should handle network error', async () => {
      ...
      await expect(sshConnect('test.example.com', 22, 'testuser', 'testpass')).rejects.toThrow(
        'Network error'
      )
    })
  ...
  describe('sshDisconnect', () => {
    ...
    it('should handle disconnect failure', async () => {
      ...
      await expect(sshDisconnect('nonexistent_conn')).rejects.toThrow('连接不存在')
    })
  })
  ...
  describe('sshList', () => {
    ...
    it('should handle list error', async () => {
      ...
      await expect(sshList('test_conn_123', '/nonexistent')).rejects.toThrow('路径不存在')
    })
  })
})
