 ...
 */
export async function loadPreviewContent(content: HTMLElement, path: string, ext: string, scale: number = 1, imageId?: string): Promise<void> {
  ...
  try {
    ...
    // Note: Audio controls are in the window toolbar, created by window.ts
      ...
      const audioMimeType = audioExt === '.mp3' ? 'audio/mpeg'
                            : audioExt === '.wav' ? 'audio/wav'
                            : audioExt === '.flac' ? 'audio/flac'
                            : audioExt === '.aac' ? 'audio/aac'
                            : audioExt === '.ogg' ? 'audio/ogg'
                            : audioExt === '.m4a' ? 'audio/mp4'
                            : 'audio/mpeg';
      previewHTML = `
