...
function createPreviewHeader(
  ...
  // Close button
  const closeBtn = createTrafficLightButton('pi-times', textColor, '关闭', () =>
    closeFloatingPreview(previewWindow)
  )
  trafficLights.appendChild(closeBtn)

  // Minimize button
  const minimizeBtn = createTrafficLightButton('pi-minus', textColor, '最小化', () =>
  ...
  if (isImage || isVideo || isAudio || isDocument || isCode || isSpreadsheet) {
    const fullscreenBtn = createTrafficLightButton('pi-window-maximize', textColor, '全屏', () =>
...
function createPreviewToolbar(
  ...
  if (isCode) {
    toolbarRight.appendChild(createToolbarSeparator())
    const codeFullscreenBtn = createToolbarButton('pi-arrows-alt', '全屏预览', () =>
      toggleFullscreen(previewWindow)
    )
  ...
  if (isSpreadsheet) {
    toolbarRight.appendChild(createToolbarSeparator())
    const spreadsheetFullscreenBtn = createToolbarButton('pi-arrows-alt', '全屏预览', () =>
      toggleFullscreen(previewWindow)
    )
