...
def test_nodes_v1():
    ...
    from ComfyUI_Data_Manager.core.nodes_v1 import (
        DataManagerCore, InputPathConfig, OutputPathConfig,
    ...
    result = instance.process(
        target_path="./output",
        file_type="image",
        file_input={"path": "/test/image.png"}
    )
    config = json.loads(result[0])
...
def test_nodes_v3_constants():
    ...
    assert "test" in directory and "output" in directory, f"目录应包含 test/output，实际为 {directory}"
    ...
    from ComfyUI_Data_Manager.core.nodes_v3 import (
        BASIC_TYPES, IMAGE_TYPES, LATENT_TYPES, MODEL_TYPES,
        MEDIA_TYPES, SAMPLER_TYPES, ADVANCED_TYPES,
        MODEL_3D_TYPES, EXTENDED_TYPES, ALL_SUPPORTED_TYPES
    )
...
def test_api_handlers():
    ...
    from ComfyUI_Data_Manager.api.routes.files import (
        list_files_handler, get_file_info_handler
    )
